...
def hybrid_search(client, query_content, query_emb, collection_name = "text_collection", top_k = 2, rerank_config= "rrf"):
    ...
    search_param_2 = {
        ...
        "param": {"drop_ratio_search": 0, "metric_type": "BM25"},
        "limit": top_k
    }
...
# do some caching to avoid excessive re-computation during experiments since we redo ranking for
# bm25, dense, and hybrid and likely have some overlap
rerank_q_doc_cache = {}
def rerank_texts(reranker_model, query_content, text_uid, top_k=2):
    paired_content_to_ids = defaultdict(lambda: [])
    for tu in text_uid:
        paired_content_to_ids[(query_content, tu[0][0])].append(tu[0][1])
    paired_content = [k for k in paired_content_to_ids.keys() if k not in rerank_q_doc_cache]
    # print(len(paired_content))
    score_dict = {}
    for content,uid_list in paired_content_to_ids.items():
        for uid in uid_list:
            if content in rerank_q_doc_cache:
                score_dict[uid] = rerank_q_doc_cache[content]
    if paired_content:
        scores = reranker_model.compute_score(paired_content, normalize=True)
        for ind in range(len(paired_content)):
            for uid in paired_content_to_ids[paired_content[ind]]:
                score_dict[uid] = scores[ind]
            rerank_q_doc_cache[paired_content[ind]] = scores[ind]
    # print(scores[0])
    # print(f"xyz, {top_k}")
...
def main(args):
    ...
    reranker_model = FlagReranker('BAAI/bge-reranker-v2-m3',
                                  use_fp16=True,
                                  device=device)
    ...
    for query_row in tqdm(query_rows, total=len(query_rows), desc="Generating query results"):
        ...
        for rerank_config in ['rrf', 'bm25_only', 'vector_only']:
            ...
            text_reranked = rerank_texts(reranker_model, query_row['query'], text_rank_list, top_k=rerank_top_k)
            ...
            table_reranked = rerank_texts(reranker_model, query_row['query'], table_rank_list, top_k=rerank_top_k)
            txt_tbl_results = {"text": text_reranked, "table": table_reranked}
